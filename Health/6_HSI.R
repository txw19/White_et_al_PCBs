...
df1<-subset(df,select=c('Treatment','time','HSI',
'Sex','Tank'))
head(df1)

# write.csv(df, 'hsi_data_for_matchin.csv')
...
sum1<-summaryBy(HSI~time+Treatment,data=df1,FUN=sumfun,na.rm=T) 
sum1
...
### CREATE BOX PLOT #####
#########################
a<-boxplot(HSI~time+Treatment,data=df1)

hist(df1$HSI,col='magenta',breaks=30)

#######################
##Log Transform Y Data#
#######################
hist(log(df1$HSI),col='magenta',breaks=30)

#Create Object with log transformed ppm values
df1$lcort<-(log(df1$HSI))
